...
export default class MarkdownFileProcessor extends FileProcessor {
  async process ({ inDir, outDir, filePath }) {
    ...
    const outPath = path.join(outDir, MarkdownFileProcessor.getOutputPath(filePath))

    const input = await fs.readFile(inPath)
    const bibliography = await findFileInParents(path.dirname(inPath), 'bibliography.bib')

    const vfile = await renderer.render({
      filePath, input, bibliography, config: this.config
    })
