...
interface FileEditorArgs<TSave> {
	tabData: TabData;
	getAutosave: () => TSave | null;
	restoreAutosave: (autosave: TSave) => void;
	getFile: () => string | null;
	restoreFile: (contents: string) => void;
	doInit?: () => void;
}

// Functions passed here mustn't depend on state variables (use refs instead).
export function useFileEditor<TSave>({
	tabData,
	getAutosave,
	restoreAutosave,
	getFile,
