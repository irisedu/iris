...
export async function findFileInParents (filePath, fileName) {
  ...
  if (filePath !== '.' && filePath !== '/') { return await findFileInParents(path.dirname(filePath), fileName) }
}
...
export async function langtoolStart (config) {
  ...
  await (async function wait () {
    try {
      const res = await langtoolCheck(config, { language: 'en-US', text: 'This is a test.' })
      if (res.status !== 200) { await wait() }
    } catch (e) {
      await wait()
    }
  })()
